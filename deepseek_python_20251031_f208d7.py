        ...
        try:
            ...
            for service_key, service_info in config.SUPPORTED_SERVICES.items():
                ...
                if service_key == 'gplinks':
                    ...
                    key_preview = config.FCLC_API[:8] + '...' if has_key else 'Not set'
                else:
                    has_key = True
    ...
    try:
        ...
        for service, info in config.SUPPORTED_SERVICES.items():
            status = "✅" if not info['requires_key'] or (
                (service == 'gplinks' and config.GPLINKS_API) or
                (service == 'shrinkearn' and config.SHRINKEARN_API) or
                (service == 'shrtfly' and config.SHRTFLY_API) or
                (service == 'fclc' and config.FCLC_API)
            ) else "❌"
