...
def shorten_all_services_sync(long_url: str) -> list:
    """Shorten URL using all services (synchronous version for thread pool)"""
    services = ['gplinks', 'shrinkearn', 'shrtfly', 'fclc']
    results = []
    
    for service in services:
        result = shorten_link_api(service, long_url)
...
async def shorten_all_services(long_url: str) -> list:
    ...
    results = await loop.run_in_executor(executor, shorten_all_services_sync, long_url)
    return results
...
async def process_all_services(long_url: str, query) -> int:
    ...
    try:
        ...
        logger.error(f"Error in process_all_services: {e}")
        await query.edit_message_text(
            f"❌ **Error Processing All Services**\n\n"
            f"An error occurred while processing your request: {str(e)}\n\n"
            f"Please try again or use individual services.",
            parse_mode='Markdown'
        )
...
async def select_service(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(f"Processing service: {service_data} for URL: {long_url}")
...
def main() -> None:
    ...
    shorten_conv_handler = ConversationHandler(
        ...
        states={
            ...
            SELECT_SERVICE: [
                # Fix: Use the correct pattern that matches all service callbacks
