...
class UPIPaymentManager:
    ...
    def generate_upi_url(self, user_id):
        ...
        # Create UPI URL for actual payment
        upi_url = f"upi://pay?pa={self.upi_id}&pn={quote(self.upi_name)}&am={amount}&tid={transaction_id}&tn=Premium%20Subscription&cu=INR"
        
        # For Telegram buttons, use a web-based UPI URL generator or direct instructions
        # Using a simple web service that converts UPI to clickable link
        button_url = f"https://mathisvaneetvelde.com/upi.html?pa={self.upi_id}&pn={quote(self.upi_name)}&am={amount}"
    ...
    def get_payment_instructions(self):
        ...
        return (
            ...
            f"1. **Click '💳 Pay via UPI' button below**\n"
            f"2. **Or manually send ₹{self.price} to UPI ID:** `{self.upi_id}`\n"
...
async def show_help_info(client, message):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("💳 Buy Premium", callback_data="buy_subscription")],
        [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu")]
    ])
    ...
    await message.edit_text(
        ...
        "**How to Use:**\n"
        "1. **Upload** any file (document/video/audio)\n"
        "2. **Wait** for automatic processing\n"
        "3. **Get** instant download & streaming links\n\n"
        "**Premium Features:**\n"
        "• Unlimited file uploads\n"
        "• High-speed transfers\n"
        "• Video streaming player\n"
        ...
        "**Max File Size:** " + humanbytes(MAX_FILE_SIZE),
        reply_markup=keyboard
    )

async def show_my_id(client, message):
    user_id = message.from_user.id
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu")]
    ])
    ...
    await message.edit_text(
        ...
        f"**User ID:** `{user_id}`\n"
        f"**Premium Status:** {'✅ Active' if is_user_premium(user_id) else '❌ Inactive'}\n\n"
        f"**Keep your User ID safe!**",
        reply_markup=keyboard
    )

async def show_upload_help(client, message):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("💳 Buy Premium", callback_data="buy_subscription")],
        [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu")]
    ])
    
    status_text = "✅ **Ready to upload!**" if is_user_premium(message.from_user.id) else "💳 **Premium subscription required!**"
    
    await message.edit_text(
        f"📁 **File Upload Guide**\n\n"
        f"{status_text}\n\n"
        "**How to Upload:**\n"
        "1. Simply send any file (document/video/audio) to this chat\n"
        "2. The bot will automatically process it\n"
        "3. You'll receive download & streaming links\n\n"
        "**Features:**\n"
...
async def show_speed_test(client, message):
    keyboard = InlineKeyboardMarkup([
    ...
    # Simulate speed test
    test_message = await message.edit_text("🚀 Running speed test...")
    await asyncio.sleep(1)
    
    # Generate random speed results for demo
    import random
    download_speed = random.uniform(50, 200)
    upload_speed = random.uniform(30, 150)
    ...
    await test_message.edit_text(
        ...
        f"**Connection:** ✅ Stable\n"
        f"**Server:** Wasabi ({WASABI_REGION})\n\n"
        f"*Note: Actual speeds may vary based on file size and network conditions.*",
        reply_markup=keyboard
    )
...
async def buy_handler(client: Client, message: Message):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("💳 Pay via UPI", url=payment_data['button_url'])],
    ...
    await message.reply_text(
        ...
        "After payment, click 'I've Paid' and forward the payment screenshot to admin.",
        reply_markup=keyboard
    )
...
async def handle_callback_query(client, callback_query):
    ...
    try:
        # Handle navigation and payment callbacks first
        if data == "main_menu":
            ...
            await callback_query.answer()
            await show_help_info(client, message)
            return
            
        elif data == "my_id":
            await callback_query.answer()
            await show_my_id(client, message)
            return
            
        elif data == "upload_help":
            await callback_query.answer()
            await show_upload_help(client, message)
            return
            
        elif data == "speed_test":
            await callback_query.answer()
            await show_speed_test(client, message)
            return
        ...
        # Handle file operation callbacks (cd_, cp_, del_, ref_)
        if '_' in data:
